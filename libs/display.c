...
static char* color_formart(int band, char* color) {
    static char string[8];
    sprintf(string, "%d - %s", band, color);
    return string;
}

static char* to_string(float value) {
    static char string[5];
    sprintf(string, "%1.0f", value);
    return string;
}
...
void display_update(float mean, float resistor, char** colors) {
    ...
    draw_string(color_formart(1, colors[0]), 8, 6);
    draw_string(color_formart(2, colors[1]), 8, 16);
    draw_string(color_formart(3, colors[2]), 8, 28);
    draw_string("ADC", 13, 41);
    draw_string("Resisten.", 50, 41);
    line(44, 37, 44, 60, color);
    draw_string(to_string(mean), 8, 52);
    draw_string(to_string(resistor), 59, 52);
    send_data();
}